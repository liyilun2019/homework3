...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        int time = Integer.parseInt(durationSelector.getText().toString());
        animator1.setDuration(time);
        animator1.setRepeatCount(ObjectAnimator.INFINITE);
        animator1.setRepeatMode(ObjectAnimator.REVERSE);

        // TODO ex2-1：在这里实现另一个 ObjectAnimator，对 target 控件的大小进行缩放，从 1 到 2 循环
        ValueAnimator scaler = ValueAnimator.ofFloat(1,2);
        scaler.setDuration(time);
        scaler.setRepeatCount(ValueAnimator.INFINITE);
        scaler.setRepeatMode(ValueAnimator.REVERSE);
        scaler.setInterpolator(new LinearInterpolator());
        scaler.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
        ...
        animatorSet.playTogether(animator1,scaler,alphaAnimator);
        animatorSet.start();
    }
}
